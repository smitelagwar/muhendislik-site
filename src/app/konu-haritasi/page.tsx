    ...
    <div className="mx-auto min-h-screen max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-12 max-w-3xl">
        ...
        <p className="mt-4 text-sm leading-7 text-zinc-600 dark:text-zinc-400 md:text-base">
          Sitedeki tüm kategori dallarını, makaleleri ve hesap araçlarını tek ekranda görün. Araçlar ayrı bir kategori olarak kalır; teknik içerikler kendi dallarında listelenir.
        </p>
      </div>
        ...
        {treeData.map(({ section, articles: sectionArticles, tools: sectionTools }) => (
          <section key={section.id} className="rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">{section.id === "araclar" ? "Araç kategorisi" : "Kategori"}</p>
            ...
            {section.id === "araclar" ? (
              <div className="grid gap-3">
                {sectionTools.map((tool) => (
                  <Link key={tool.id} href={tool.href} className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4 transition-colors hover:border-blue-300 hover:bg-white dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-blue-800 dark:hover:bg-zinc-900">
              ...
              <div className="grid gap-3">
                {sectionArticles.map((article) => (
                  <Link key={article.slug} href={`/${article.slug}`} className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4 transition-colors hover:border-blue-300 hover:bg-white dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-blue-800 dark:hover:bg-zinc-900">
                    <div className="flex items-center gap-3">
      ...
      <section className="mt-10 rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          {STATIC_PAGES.map((page) => (
            <Link key={page.href} href={page.href} className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4 text-sm font-bold text-zinc-800 transition-colors hover:border-blue-300 hover:text-blue-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:border-blue-800 dark:hover:text-blue-300">
              {page.label}
            </Link>
          ))}
        </div>
