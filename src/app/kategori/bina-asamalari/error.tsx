...
import { AlertTriangle, GitBranchPlus, Home, RefreshCw, Route } from "lucide-react";
import { RouteFeedback } from "@/components/route-feedback";
import { Button } from "@/components/ui/button";

interface BinaAsamalariErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function BinaAsamalariError({ error, reset }: BinaAsamalariErrorProps) {
  return (
    <RouteFeedback
      eyebrow="Bina aşamaları uyarısı"
      title="Bina aşaması içeriği yüklenemedi"
      description="İnşaat süreci rehberi beklenen içerik verisini üretemedi. Yeniden deneyebilir veya bina aşamaları ana sayfasına dönebilirsiniz."
      actions={[
      ...
      <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-4 text-sm leading-7 text-red-100">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <p>
              Rehber sayfası güvenli hata ekranına alındı; veri üretimi yeniden denenebilir.
