...
function stiffnessDiagram(spec: DepremRolloutSpec) {
  ...
  const body = `<rect x="350" y="155" width="500" height="370" rx="14" fill="${WHITE}" stroke="${NAVY}" stroke-width="6"/>${rebars}<line x1="350" y1="335" x2="850" y2="335" stroke="${CYAN}" stroke-width="5" stroke-dasharray="14 10"/><text x="865" y="343" class="accent">Tarafsız Eksen</text><path d="M365 360h470v150H365z" fill="url(#hatch)"/><path d="M410 440l28-32 24 25 28-42 26 38 30-46 25 40 30-28 26 35" fill="none" stroke="${CYAN}" stroke-width="4"/><text x="455" y="115" class="label">Çatlamış Kesit</text><text x="515" y="590" class="accent">EIetkin</text>`;
  return shell(spec.headline, "Tek betonarme kesitte çatlaklı bölge, tarafsız eksen ve etkin eğilme rijitliği kavramını gösteren teknik kesit detayı.", body);
}
...
function massDiagram(spec: DepremRolloutSpec) {
  const floors = [170, 270, 370, 470].map((y) => `<line x1="340" y1="${y}" x2="820" y2="${y}" stroke="${NAVY}" stroke-width="6"/><circle cx="580" cy="${y - 18}" r="23" fill="${CYAN}" fill-opacity="0.12" stroke="${CYAN}" stroke-width="4"/>`).join("");
  const body = `<line x1="390" y1="135" x2="390" y2="505" stroke="${NAVY}" stroke-width="6"/><line x1="770" y1="135" x2="770" y2="505" stroke="${NAVY}" stroke-width="6"/>${floors}<path d="M300 530H860" stroke="${NAVY}" stroke-width="7"/><text x="865" y="125" class="accent">Kat Kütlesi</text><line x1="820" y1="170" x2="935" y2="170" stroke="${CYAN}" stroke-width="4" marker-end="url(#arrow)"/><line x1="820" y1="270" x2="935" y2="270" stroke="${CYAN}" stroke-width="4" marker-end="url(#arrow)"/><line x1="820" y1="370" x2="935" y2="370" stroke="${CYAN}" stroke-width="4" marker-end="url(#arrow)"/><line x1="820" y1="470" x2="935" y2="470" stroke="${CYAN}" stroke-width="4" marker-end="url(#arrow)"/><text x="270" y="115" class="label">Diyaframlar</text><text x="805" y="585" class="label">Modal Kütle</text>`;
  return shell(spec.headline, "Kat diyaframlarına dağıtılan eşit sembolik kütle noktalarının modal deprem hesabına taşındığını gösteren tek bina elevasyonu teknik detayı.", body);
}
