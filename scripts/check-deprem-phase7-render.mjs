...
const targets = [
  ...
  { slug: "eurocode-ts-en-1991-1-3-kar-yuku-hesabi-bolge-haritasi-ile", expected: "0,96 kN/m²" },
  { slug: "eurocode-ts-en-1991-1-4-ruzgar-yuku-hesabi-turkiye-bolgeleri", expected: "-1,12 kN/m²" },
  { slug: "eurocode-ts-en-1992-1-1-ec2-ts-500-ile-karsilastirmali-analiz", expected: "TS 500" },
];
...
try {
    ...
    for (const theme of ["light", "dark"]) {
      for (const viewport of viewports) {
        ...
        try {
          ...
          assert(result.links.some((href) => ["mevzuat.gov.tr", "resmigazete.gov.tr", "csb.gov.tr", "europa.eu"].some((domain) => href.includes(domain))), `${target.slug}: resmî kaynak linki görünmüyor.`);
  ...
  console.log(JSON.stringify({
    status: "ok",
    phase: "FAZ 7 — BEP/TS825 + Akustik + Eurocode",
    routes: targets.length,
    checks: completed.length,
    matrix: "12 routes × 2 themes × 2 viewports",
