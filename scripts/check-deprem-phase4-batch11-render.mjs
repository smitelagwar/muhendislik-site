...
const targets = [
  { slug: "yapi-denetimi-ts708-donati-celigi-kabul", expected: "TS EN ISO 6892-1" },
  { slug: "yapi-denetimi-en13670-yerlestirme-kur-tolerans", expected: "TS EN 13670" },
];
const viewports = [
...
try {
  ...
  console.log(JSON.stringify({ status: "ok", phase: "FAZ 4 batch 11", routes: targets.length, checks: completed.length, matrix: "2 routes × 2 themes × 2 viewports", classifications: Object.fromEntries(targets.map((target) => [target.slug, "C3"])), completed }, null, 2));
} finally {
  await browser.close();
