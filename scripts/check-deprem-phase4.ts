...
const PHASE4_SLUGS = [...BATCH_1_SLUGS, ...BATCH_2_SLUGS, ...BATCH_3_SLUGS] as const;
...
assert(DEPREM_PHASE4_ARTICLES.length === 11, `FAZ 4 override sayısı 11 olmalı; bulunan ${DEPREM_PHASE4_ARTICLES.length}.`);
assert(DEPREM_PHASE4_SLUGS.size === 11, "FAZ 4 source-of-truth slug kümesinde tekrar/eksik kayıt var.");
...
for (const slug of PHASE4_SLUGS) {
  ...
  assert(article.seriesId === "mevcut-guclendirme", `FAZ 4 Batch 1-3 seriesId mevcut-guclendirme olmalı: ${slug}`);
  ...
  assert(blocks.filter((block) => block.type === "formula").length === 0, `FAZ 4 Batch 1-3'te gereksiz FormulaBlock bulundu: ${slug}`);
}

if (errors.length > 0) {
  console.error("Deprem FAZ 4 Batch 1-3 kontrolü başarısız:\n");
...
console.log(JSON.stringify({
  ...
  batch3Classification: Object.fromEntries(BATCH_3_SLUGS.map((slug) => [slug, "C3"])),
  sourceOfTruth: "src/lib/deprem-phase4-articles.ts",
  officialSourceProfile: "AFAD TBDY 2018 Bölüm 15 + Bölüm 16 resmî kaynakları",
  visualContract: "existing unique rollout cover + body figure preserved",
  seriesCoverage: { "mevcut-guclendirme": 11, "su-zemin": 0, "yapi-denetimi": 0 },
