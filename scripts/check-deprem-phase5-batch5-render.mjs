...
const targets = [
  { slug: "cevre-ced-zorunlulugu-proje-buyuklugu-esikleri", expected: "2026/4" },
  { slug: "cevre-insaat-atigi-yonetimi-yonetmeligi", expected: "Madde 23" },
  { slug: "cevre-gurultu-ve-toz-santiye-yukumlulukleri", expected: "100 dBC" },
  { slug: "cevre-yagmur-suyu-kirliligi-ve-santiye-filtrasyonu", expected: "Mogan" },
];
const viewports = [
    ...
    for (const theme of ["light", "dark"]) {
      for (const viewport of viewports) {
        ...
        try {
          ...
          assert(result.links.some((href) => href.includes("csb.gov.tr") || href.includes("resmigazete.gov.tr")), `${target.slug}: resmî çevre kaynağı görünmüyor.`);
          assert(!result.links.some((href) => href.includes("csgb.gov.tr")), `${target.slug}: Çevre rotasında İSG kaynak profili görünüyor.`);
