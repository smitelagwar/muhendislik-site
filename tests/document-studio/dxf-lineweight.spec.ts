...
test.describe("DXF lineweight display", () => {
  ...
  test("LWT resolves source/BYLAYER/BYBLOCK widths without changing camera, layers or zoom thickness", async ({ page }, testInfo) => {
    ...
    for (const expected of [13, 35, 50, 70, 100]) {
      expect(sourceSnapshot.distinctLineweights).toContain(expected);
    }
    ...
    // Layer visibility must hide the wide overlay together with its source line object.
    ...
    // Screen-space LWT should remain approximately constant when zoom changes.
    ...
    // Stage 1 color mode and Stage 2 LWT are independent controls.
