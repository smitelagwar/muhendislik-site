...
const targets = [
  { slug: "su-yalitimi-ts-4749-uygulama-detaylari", expected: "k ≥ 10^-4 m/s" },
  { slug: "yagmur-suyu-drenaji-ve-sizma-tesisi-hesabi", expected: "TS EN 16941-1" },
];
const viewports = [
...
try {
  ...
  console.log(JSON.stringify({ status: "ok", phase: "FAZ 4 batch 8", routes: targets.length, checks: completed.length, matrix: "2 routes × 2 themes × 2 viewports", classifications: Object.fromEntries(targets.map((target) => [target.slug, "C3"])), completed }, null, 2));
} finally {
  await browser.close();
