...
function CalloutBox({ content, tone }: { content: string; tone: string }) {
  const isWarning = /(uyarı|warning|important)/i.test(tone);
...
function TableViewer({ content }: { content: string }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {fullscreen ? (
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[130] flex flex-col bg-white p-4 dark:bg-zinc-950 md:p-8">
    ...
    <LazyMotion features={domAnimation}>
      <div className="relative flex flex-col">
          ...
          <article className="mx-auto w-full max-w-3xl lg:w-8/12 xl:mx-0">
            ...
            <header className="mb-10">
              <div className="mb-6 flex flex-wrap gap-2">
                ...
                <Badge variant="outline" className="border-zinc-200 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:border-zinc-800">{article.badgeLabel}</Badge>
              </div>
            ...
            <div className="prose prose-zinc max-w-none dark:prose-invert prose-headings:font-black prose-headings:tracking-tight prose-h2:mt-16 prose-h2:border-b-2 prose-h2:border-zinc-100 prose-h2:pb-4 prose-h2:text-3xl dark:prose-h2:border-zinc-800 prose-h3:mt-10 prose-h3:text-2xl prose-p:mb-8 prose-p:max-w-[72ch] prose-p:text-[1.02em] prose-p:leading-[1.72] prose-li:leading-[1.72] prose-li:marker:text-blue-600 prose-a:font-bold prose-a:text-blue-700 prose-a:no-underline hover:prose-a:underline dark:prose-a:text-blue-400 prose-strong:font-black prose-strong:text-zinc-900 dark:prose-strong:text-zinc-50" style={{ fontSize: "var(--article-font-size, 17px)" }}>
              {parsedSections.map((sectionItem, sectionIndex) => (
                <div key={sectionItem.id}>
                  <h2 id={sectionItem.id} className="scroll-m-32">{sectionItem.title}</h2>
                  {sectionItem.blocks.map((block, blockIndex) => {
                    ...
                    if (block.type === "list") {
                      ...
                      return (
                        <ListTag key={`${sectionItem.id}-${blockIndex}`} className={ordered ? "my-8 list-decimal space-y-3 pl-6" : "my-8 list-disc space-y-3 pl-6"}>
                          {items.map((item, itemIndex) => <li key={itemIndex} dangerouslySetInnerHTML={renderInlineMarkdown(item.replace(/^(?:- |\d+\.\s)/, ""))} />)}
                        </ListTag>
                      );
                    }
                    ...
                    if (block.type === "quote") return <blockquote key={`${sectionItem.id}-${blockIndex}`} className="border-l-4 border-blue-600 bg-blue-50/40 px-6 py-4 italic dark:bg-blue-950/10"><div dangerouslySetInnerHTML={renderInlineMarkdown(block.content)} /></blockquote>;
              ...
              {relatedArticles.length > 0 ? (
                <div className="not-prose mb-12 mt-4">
                  ...
                  <Link href={`/${relatedArticles[0].slug}`} className="group block overflow-hidden rounded-3xl border border-blue-200/50 bg-gradient-to-br from-blue-50 to-indigo-50 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-500/10 dark:border-blue-800/30 dark:from-blue-950/40 dark:to-indigo-950/40">
                    <div className="flex flex-col sm:flex-row">
                      <div className="relative aspect-video w-full flex-shrink-0 overflow-hidden sm:h-auto sm:w-48 sm:aspect-auto">
                        <Image src={relatedArticles[0].image} alt={relatedArticles[0].title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 640px) 100vw, 200px" />
                      </div>
                      <div className="flex flex-1 flex-col justify-center p-6">
                        <Badge variant="outline" className={`${relatedArticles[0].categoryColor} mb-2 w-fit border-none text-[9px] font-black uppercase`}>{relatedArticles[0].category}</Badge>
                        <h4 className="mb-2 text-lg font-black leading-snug text-zinc-900 transition-colors group-hover:text-blue-700 dark:text-zinc-100 dark:group-hover:text-blue-400">{relatedArticles[0].title}</h4>
                        <p className="line-clamp-2 text-sm text-zinc-500 dark:text-zinc-400">{relatedArticles[0].description}</p>
        ...
        <AnimatePresence>
          {mobileTocOpen ? (
            <>
              <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm lg:hidden" onClick={() => setMobileTocOpen(false)} />
              <m.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 210 }} className="fixed bottom-0 left-0 right-0 z-[60] flex max-h-[85vh] flex-col rounded-t-3xl border-t border-zinc-200 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.1)] dark:border-zinc-800 dark:bg-zinc-950 lg:hidden">
                <div className="flex flex-shrink-0 items-center justify-between border-b border-zinc-100 p-6 dark:border-zinc-800">
                  <h3 className="flex items-center gap-2 text-lg font-black"><BookOpen className="h-5 w-5 text-blue-600" /> İçindekiler</h3>
