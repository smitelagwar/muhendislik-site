...
const Context = createContext<{ manager: UploadQueueManager; queue: UIQueueItem[] } | null>(null);
  ...
  const runQueueItem = async (item: UploadQueueItem, onProgress: (value: number) => void) => {
    if (!item.file) return;
    ...
    try {
      ...
      if (tokenData.isLocalMode) {
        ...
        onProgress(10);
        await (await import("@vercel/blob/client")).upload(tokenData.pathname, file, {
          access: "private",
          ...
          onUploadProgress: ({ percentage }) => {
            onProgress(Math.min(90, Math.max(10, Math.round(percentage * .8 + 10))));
          },
        });
        onProgress(92);
      }
...
export function DokWorkspaceSessionProvider({children}: {children: React.ReactNode}) {
  const [manager] = useState(() => { const m = new UploadQueueManager(3); m.setExecutor(runQueueItem); return m; });
  const [queue, setQueue] = useState<UIQueueItem[]>([]);
  useEffect(() => manager.subscribe(items => setQueue(items.map(item => ({ ...item, status: item.status === "success" ? "completed" : item.status === "failed" || item.status === "cancelled" ? "error" : item.status })))), [manager]);
  return <DokQueryProvider><Context.Provider value={{manager,queue}}>{children}</Context.Provider></DokQueryProvider>;
}
