...
15 deponun repo URL'si, branch'i, HEAD SHA'sı, commit tarihi, pushedAt ve archived alanları [snapshot JSON'unda](arastirma__github-snapshot.json) kayıtlı. Bunlar tekrar üretilebilir başlangıç kimlikleri; star sayısı veya son commit tarihi kalite garantisi değildir. Native/WASM bridge'in upstream'e ne kadar geride olduğu, örnek dosyalara verilen yanıtlar, güvenlik düzeltme süresi ve kaynak derlemenin tekrar üretilebilirliği daha anlamlıdır.

Sınırlı açık issue örneklemi [ayrı JSON'da](arastirma__github-issue-ornekleri.json): MLightCAD'de [SHX yükleme konusu](https://github.com/mlightcad/cad-viewer/issues/571); dxf-viewer'da [parse sırasında takılma bildirimi](https://github.com/vagran/dxf-viewer/issues/166) ve [çok satırlı tablo metni bildirimi](https://github.com/vagran/dxf-viewer/issues/164) bulundu. Yalnız başlık/metadata incelendi; burada tekrar üretilmiş kusur veya bizim kurulu sürümümüzün hatası diye sunulmuyor. ACadSharp'ın örnek API sayfasında filtre sonrası sıfır issue kalması, projede açık issue olmadığı anlamına gelmiyor; sayfa PR'lar da içeriyordu.
