...
const targets = [
  { slug: "zemin-etudu-minimum-sondaj-sayisi-ve-derinligi", expected: "300 m²" },
  { slug: "tbdy-bolum-16-zemin-yapi-etkilesimi", expected: "kinematik etkileşim" },
  { slug: "zemin-sivlasma-riski-degerlendirmesi", expected: "N1,60" },
];
const viewports = [
...
try {
  ...
  console.log(JSON.stringify({ status: "ok", phase: "FAZ 4 batch 7", routes: targets.length, checks: completed.length, matrix: "3 routes × 2 themes × 2 viewports", classifications: Object.fromEntries(targets.map((target) => [target.slug, "C3"])), completed }, null, 2));
} finally {
  await browser.close();
