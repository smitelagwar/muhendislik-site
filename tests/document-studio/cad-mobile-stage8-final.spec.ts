...
async function uploadFixture(page: Page): Promise<string> {
  const fixture = createPrecisionFixture();
  return page.evaluate(async ({ content }) => {
    const formData = new FormData();
    formData.append(
      "file",
      new File([content], "stage8-final-mobile-precision.dxf", {
        type: "application/dxf",
      })
    );
    formData.append("pathname", `cad-stage8-final-${crypto.randomUUID()}.dxf`);
    ...
    if (!response.ok || !payload.file?.id) {
      throw new Error(payload.error || "Stage 8 final DXF fixture yüklenemedi");
    }
...
async function openViewer(page: Page): Promise<ViewerHarness> {
  ...
  if (!box) throw new Error("Stage 8 final CAD viewport ölçülemedi");
  return {
    host,
    viewport,
...
async function openSnapPanel(page: Page): Promise<Locator> {
...
async function closeSnapPanel(panel: Locator): Promise<void> {
...
async function setSnapMaster(page: Page, enabled: boolean): Promise<void> {
  const panel = await openSnapPanel(page);
...
async function longPressUntilTracking(
  ...
  await dispatchTouchPointer(viewport, "pointerdown", point, pointerId);
...
async function completedLineGeometry(page: Page): Promise<{
  ...
  const coordinates = await line.evaluate((node) => {
...
test("Stage 8 final: beş snap modu gerçek CAD runtime üzerinde ayrı ayrı çözülür ve ayar kalıcıdır", async ({ page }) => {
...
test("Stage 8 final: long-press eşiği, büyüteç, offset crosshair, rubber-band ve iki nokta commit birlikte çalışır", async ({ page }) => {
  const { host, viewport, center } = await openViewer(page);
  await setOnlySnapMode(page, "intersection");
  await startDistance(page, host);

  await longPressUntilTracking(page, viewport, host, center, 61, "tracking-first");
  await expect(page.getByTestId("cad-snap-label")).toHaveAttribute(
    "data-cad-snap-label",
    "intersection"
  );
  ...
  if (!lensBox || !viewportBox) throw new Error("Stage 8 final büyüteç/viewport ölçülemedi");
  ...
  await dispatchTouchPointer(viewport, "pointermove", offsetPoint, 61);
  await expect(page.locator('[data-cad-offset-guide="true"]')).toBeAttached();
  await dispatchTouchPointer(viewport, "pointerup", offsetPoint, 61);
  ...
  await longPressUntilTracking(page, viewport, host, second, 62, "tracking-second");
  await expect(page.locator('[data-cad-distance-rubber-band="true"]')).toBeAttached();
  await dispatchTouchPointer(viewport, "pointermove", { x: second.x + 15, y: second.y + 5 }, 62);
  await dispatchTouchPointer(viewport, "pointerup", { x: second.x + 15, y: second.y + 5 }, 62);

  await expect(host).toHaveAttribute("data-cad-active-tool", "none");
  await expect(page.locator('[data-cad-distance-complete="true"]')).toHaveCount(1);
  await expect(page.getByTestId("cad-precision-magnifier")).toHaveCount(0);
  expect((await completedLineGeometry(page)).length).toBeGreaterThan(50);
});

test("Stage 8 final: ikinci parmak pending long-press ölçümünü iptal eder", async ({ page, browserName }) => {
  test.skip(
    browserName === "webkit",
    "Playwright WebKit sentetik PointerEvent akışı eşzamanlı iki touch pointer durumunu güvenilir biçimde korumuyor; davranış Chromium browser kapısı ve Stage 3 gesture regresyonuyla doğrulanıyor."
  );
...
test("Stage 8 final: trusted iki-parmak pinch doğal hızda zoom yapar ve merkez ankrajını korur", async ({ page, browserName }) => {
  test.skip(
    browserName !== "chromium",
    "Trusted multi-touch injection Playwright WebKit'te CDP üzerinden kullanılamıyor; pinch kamera kapısı Chromium/CDP ile doğrulanıyor."
  );
  ...
  const second = { x: center.x + 90, y: center.y };
  await longPressUntilTracking(page, viewport, host, second, 82, "tracking-second");
  await dispatchTouchPointer(viewport, "pointerup", second, 82);
  await expect(host).toHaveAttribute("data-cad-active-tool", "none");
