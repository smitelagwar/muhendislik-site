...
console.log(JSON.stringify({ status: "ok", phase: "FAZ 6", completedBatches: 2, phase6Overrides: DEPREM_PHASE6_ARTICLES.length, phase2C1CarryForward: [C1_IMAR_PILOT], completedArticles: 9, targetArticles: 22, remaining: 13, batch1C3Slugs: IMAR_BATCH_1_C3_SLUGS, batch2C3Slugs: IMAR_BATCH_2_C3_SLUGS, classifications: { ...Object.fromEntries(IMAR_C3_SLUGS.map((slug) => [slug, "C3"])), [C1_IMAR_PILOT]: "C1" }, sourceOfTruth: "src/lib/deprem-phase6-articles.ts + preserved FAZ 2 C1 İmar pilot", officialSourceProfile: "İmar: Planlı Alanlar İmar Yönetmeliği + 3194 sayılı İmar Kanunu + Mekânsal Planlar Yapım Yönetmeliği + 2026 resmî değişiklik zinciri", visualContract: "8 rollout unique cover/body figure + 1 preserved FAZ 2 pilot unique cover/body figure", qualityScoreContract: "static subtotal >=80/90 + responsive layout QA 10/10 => final >=90/100; hard-fail assertions mandatory", qualityScores, seriesCoverage: { imar: 9, otopark: 0, asansor: 0, engelsiz: 0 }, targetCoverage: { imar: 9, otopark: 5, asansor: 4, engelsiz: 4, total: 22 }, ts500Touched: false }, null, 2));
