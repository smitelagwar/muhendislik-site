...
function buildOperationsSection(guide: BinaGuideData, sectionNumber: number): BinaGuideSection {
  ...
  return {
    ...
    subsections: [],
    content: [
      `${guide.title}, cogu projede teknik olarak dogru tarif edildigi halde saha operasyonu gevsek kuruldugu icin kalite kaybi ureten bir kalemdir. Bu nedenle muhendis icin asil is, imalati yalniz baslatmak degil; baslangic onayi, gunluk takip, kapatma oncesi kontrol ve teslim onayi arasinda kopmayan bir zincir kurmaktir.`,
      `${guide.category} paketi icinde bu konu genellikle baska ekiplerle temas halindedir. O nedenle aks, kot, detay, rezervasyon, gizlenecek noktalar ve test sirasi ise baslamadan once netlesmezse sorun teknik eksikten cok gec yakalanan koordinasyon zafiyeti olarak ortaya cikar. Sahadaki tekrar islerin buyuk bolumu de tam bu asamada dogar.`,
      ...
      table(
        ...
        [
          ["Baslangic", "Aks, kot, detay, rezervasyon ve numune karari", "Saha muhendisi + formen", "Ise girmeden once"],
      ...
      connectedPaths.length > 0
        ...
        : "",
      "Saha ipucu: uzeri kapanacak veya sonradan erisimi zorlasacak her detay ayni gun icinde olcu, fotograf ve gerekiyorsa test kaydiyla kapatilmalidir. Aksi halde sorun fark edildiginde dogru sebebi ispat etmek zorlugu baslar.",
    ]
      .filter(Boolean)
      .join("\n\n"),
  };
...
function buildFieldTipsSection(guide: BinaGuideData, sectionNumber: number): BinaGuideSection {
  return {
    ...
    subsections: [],
    content: [
      `${guide.title.toLocaleLowerCase("tr-TR")} kaleminde hizli gorunen cozum ile saglikli cozum her zaman ayni degildir. Ekipleri erken sahaya sokmak bazen takvimi hizlandiriyor gibi gorunur; fakat onceki kalemin kabulunu tamamlamadan yapilan baslangiclar, ayni mahale iki veya uc kere geri donulmesine yol acar. Bu da hem maliyeti hem de kaliteyi bozar.`,
      ...
      "Ozellikle teslime yaklasan projelerde ekiplerin dikkat dagilimi artar. Bu safhada yapilacak en dogru is, acik mahal yerine kapanacak mahal mantigina gecmek, eksik listesini blok bazli yonetmek ve ayni kusurun baska katlarda tekrarlanmasini engellemektir. Teslime yaklasmak kalite esigini dusurmek icin degil, kontrol disiplinini daha da keskinlestirmek icin bir sinyaldir.",
      "Sahadan hizli ama etkili ipuclari:",
      bullets([
        "Numune kararini verdikten sonra ekipler arasi yorum farkina izin verme; ayni detay tum blokta ayni sekilde tekrar etsin.",
        "Gizli kalan isleri bir sonraki ekip gelmeden once kapat; bir ust kalem gelince onceki hatayi duzeltmek pahali hale gelir.",
        "Malzeme kabulunu sadece irsaliye ile degil, uygulama noktasinda dogru urun ve dogru aksesuar eslesmesiyle yap.",
        "Bir mahalde tekrar is basladiysa bunu lokal hata gibi degil, surec hatasi gibi incele.",
      ...
      table(
        ...
        [
          ["Erken baslangic", "Bir onceki kalem eksikken ekip giriyor", "Tekrar is ve ton farki", "Hazir olmayan mahali takvimde ayri tutmak"],
          ["Eksik kayit", "Fotograf ve test tutanagi yok", "Kusurun kaynagi belirsiz kalir", "Ayni gun saha kaydi almak"],
...
function buildCloseoutSection(guide: BinaGuideData, sectionNumber: number): BinaGuideSection {
  ...
  return {
    ...
    subsections: [],
    content: [
      `${guide.title} icin son kabul, yalniz imalat bitti notu dusmekle tamamlanmaz. Kalemin hangi standart, hangi saha kaydi ve hangi son kontrol listesiyle kapatildigi acik degilse teslim formel gorunur ama gercekte eksik kalir. Bu nedenle saha muhendisi, fiziki imalat ile evrak zincirini ayni anda tamamlamalidir.`,
      "Ozellikle teknik disiplinler arasinda baglantili kalan kalemlerde as-built kaydi, son fotograf, test sonucu, revizyon notu ve eksik listesi ayni klasorde toplanmalidir. Bu paket olmadan sonradan cikacak ariza veya kullanici geri bildirimi dogru kaynaga baglanamaz.",
      "Son kontrol akisini su duzende yonetmek pratikte ise yarar:",
      ordered([
        "Fiziki imalatin bitis durumunu mahal veya aks bazinda teyit et.",
        "Gizli kalmis detaylarin fotograf, olcu veya test kayitlarini dosyaya bagla.",
        "Baglantili disiplinlerle capraz kontrol turu yap ve acik noktayi tek listede topla.",
        "Son kullanici veya isletme tarafini etkileyecek noktalari etiket, plan veya not ile gorunur hale getir.",
      ...
      table(
        ...
        [
          ...
          ["Eksik listesi kapanis formu", "Acilan kusurlarin takibi", "Santiye sefi ve isveren temsilcisi"],
