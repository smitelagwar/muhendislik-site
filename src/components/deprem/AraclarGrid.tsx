        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {araclar.map((arac, i) => (
            <Link key={i} href={arac.href} className="group">
              <Card className="h-full bg-gray-900 border-gray-800 border-l-4 border-l-amber-500 hover:border-amber-500 hover:shadow-[0_0_15px_rgba(245,158,11,0.15)] transition-all duration-300">
                <CardHeader>
                  <div className="mb-4 bg-gray-950/50 w-12 h-12 rounded-lg flex items-center justify-center border border-gray-800 group-hover:border-amber-500/50 transition-colors">
                    {arac.icon}
                  </div>
                  <CardTitle className="text-gray-100 group-hover:text-amber-500 transition-colors">{arac.title}</CardTitle>
                  <CardDescription className="text-gray-400 mt-2 line-clamp-3">
                    {arac.description}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center text-sm font-medium text-amber-500 group-hover:text-amber-400 transition-colors">
