...
const targets = [
  { slug: "yapi-denetimi-beton-tanimlama-en206-ts13515", expected: "çevresel etki sınıfı" },
  { slug: "yapi-denetimi-ebis-beton-numunesi-kabul", expected: "2022/07 Genelgesi" },
  { slug: "yapi-denetimi-dusuk-beton-dayanimi-karot", expected: "TS EN 13791" },
];
const viewports = [
...
try {
  ...
  console.log(JSON.stringify({ status: "ok", phase: "FAZ 4 batch 10", routes: targets.length, checks: completed.length, matrix: "3 routes × 2 themes × 2 viewports", classifications: Object.fromEntries(targets.map((target) => [target.slug, "C3"])), completed }, null, 2));
} finally {
  await browser.close();
