...
export function BetonDokumStudio({
  ...
  const filledFieldCount = useMemo(
    () =>
      countFilledEditableFields(
        formData,
        DOCUMENT_EDITABLE_FIELDS["beton-dokum-tutanagi"]
      ),
    [formData]
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          <span className="rounded-full bg-amber-500/20 px-1.5 py-0.2 text-[10px] font-mono text-amber-800 dark:text-amber-300">
            {filledFieldCount}/{DOCUMENT_EDITABLE_FIELDS["beton-dokum-tutanagi"].length}
          </span>
        </button>
      ...
      <div className={BELGE_STUDIO_MAIN_SPLIT_CLASS}>
        ...
        <div
          ...
          className={`flex-1 min-h-0 min-w-0 flex flex-col justify-between bg-zinc-900/10 dark:bg-zinc-950/40 p-2 sm:p-2.5 overflow-hidden ${activeTabMobile === "preview" ? "flex" : "hidden lg:flex"
            }`}
        >
