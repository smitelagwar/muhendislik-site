...
// Layout içindeki viewer gidiş/dönüşünde aynı oturumun konumunu korur.
const scrollPositions = new Map<
  string,
  { id: string; offset: number; scrollTop: number }
>();
export type MobileBucket = {
  key: string;
  label: string;
  folders: DokFolder[];
  files: DokFile[];
...
export function mobileMetadata(item: Item) {
  ...
  return "parent_id" in item
    ? `Klasör · ${label}`
    : `${item.extension.toLocaleUpperCase("tr-TR")} · ${formatBytes(item.size_bytes)} · ${label}`;
}
export function MobileExplorer({
  ...
  const blocks = useMemo(() => {
    ...
    for (const bucket of collections) {
      ...
      for (let i = 0; i < entries.length; i += step)
        result.push({
          key: `${bucket.key}:${entries[i].id}`,
          items: entries.slice(i, i + step),
        });
    }
    return result;
  ...
  const virtualizer = useVirtualizer({
    ...
    initialOffset: () => scrollPositions.get(persistenceKey)?.scrollTop ?? 0,
  ...
  const anchor = useRef<{
    ...
    offset: number;
    scrollTop: number;
  } | null>(scrollPositions.get(persistenceKey) ?? null);
  ...
  useLayoutEffect(() => {
    ...
    const save = () => {
      ...
      if (row) {
        anchor.current = {
          id: blocks[row.index].items[0]?.id || blocks[row.index].key,
          offset: el.scrollTop - row.start,
  ...
  useLayoutEffect(() => {
    const saved =
      previous.current === persistenceKey
        ? anchor.current
        : positions.current.get(persistenceKey);
    previous.current = persistenceKey;
    virtualizer.measure();
    if (saved) {
      const i = blocks.findIndex(
        (b) =>
          b.key === saved.id || b.items.some((item) => item.id === saved.id),
      );
      if (i >= 0) {
        virtualizer.scrollToIndex(i, { align: "start" });
        const row = virtualizer.getVirtualItems().find((r) => r.index === i);
        if (row) virtualizer.scrollToOffset(row.start + saved.offset);
      }
    } else if (scrollRef.current) scrollRef.current.scrollTop = 0;
  }, [view, grouped, persistenceKey, blocks, virtualizer, scrollRef]);
  const render = (item: Item) => {
    ...
    const controls = selection ? (
      <button
        ...
        aria-label={`${name} ${selectedIds.has(item.id) ? "seçimini kaldır" : "seç"}`}
        onClick={(e) => onActivate(item, "select-control", e)}
      >
      ...
      <button
        ...
        data-testid="dok-item-more"
        onClick={(e) => {
          e.stopPropagation();
          // Safari dokunmayla button'a odak vermez; panel kapanış hedefini koru.
    ...
    return (
      <div
        ...
        data-file-id={folder ? undefined : item.id}
        data-folder-id={folder ? item.id : undefined}
        data-selected={selectedIds.has(item.id)}
        data-mobile-item
        className={view === "list" ? s.row : s.tile}
        onClick={(e) => onActivate(item, "body", e)}
        onDoubleClick={(e) => onActivate(item, "double", e)}
        onContextMenu={(e) => onActivate(item, "context", e)}
      >
        {view === "grid" ? (
        ...
        <div className={s.content}>
          {folder ? (
            <button
              className={s.name}
              data-testid="dok-folder-name"
              onClick={(e) => onActivate(item, "name", e)}
            >
              {name}
            </button>
          ) : (
            <Link
              href={`/dokumantasyon/dosya/${item.id}`}
              className={s.name}
  ...
  return (
    <div
      ...
      data-testid="dok-mobile-virtual-list"
    >
      {virtualizer.getVirtualItems().map((row) => {
        ...
        return (
          ...
          >
            {block.label ? (
              <div className={s.group}>{block.label}</div>
            ) : (
              block.items.map(render)
            )}
          </div>
        );
      })}
