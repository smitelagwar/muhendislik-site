  ...
  return (
    <div ref={shellRef} className={`relative min-h-[calc(100vh-4.5rem)] w-full bg-gradient-to-b from-amber-500/[0.03] via-background to-amber-500/[0.05] max-lg:min-h-0 max-lg:overflow-hidden ${layoutStyles.shell}`}>
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1920px] px-2 py-3 lg:space-y-4 sm:px-4 sm:py-4 lg:px-6 xl:px-8 max-lg:flex max-lg:h-full max-lg:min-h-0 max-lg:flex-col max-lg:gap-3 sm:max-lg:gap-4">
        {/* Üst Bar / Header */}
        <div data-testid="dok-admin-header" className={`flex flex-col gap-3 rounded-2xl p-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5 max-lg:shrink-0 ${styles.workspaceHeader}`}>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {readiness && (
                  readiness.storageMode === "durable" ? (
                    <span data-admin-meta className="inline-flex max-w-full basis-full items-center gap-1 whitespace-normal rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 sm:basis-auto">
                      <ShieldCheck className="h-3 w-3" />
                      <span>Kalıcı Depolama (Neon + Blob)</span>
                    </span>
                  ) : readiness.storageMode === "local_dev" ? (
                    <span data-admin-meta className="inline-flex max-w-full basis-full items-center gap-1 whitespace-normal rounded border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[11px] font-semibold text-amber-600 dark:text-amber-400 sm:basis-auto">
                      <Laptop className="h-3 w-3" />
                      <span>Yerel Geliştirme (Local)</span>
                    </span>
                  ) : (
                    <span data-admin-meta className="inline-flex max-w-full basis-full items-center gap-1 whitespace-normal rounded border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[11px] font-semibold text-red-500 sm:basis-auto">
          ...
          {/* Kullanıcı Rozeti */}
          <div data-admin-user className="flex items-center justify-between gap-2 border-t border-border/60 pt-3 sm:justify-end sm:border-t-0 sm:pt-0">
