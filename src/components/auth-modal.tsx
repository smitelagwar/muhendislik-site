...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (!emailRegex.test(email)) {
      toast.error("Geçersiz e-posta formatı. Lütfen geçerli bir adres girin.");
      return;
    }
        ...
        <>
          ...
          <div className="pointer-events-none fixed inset-0 z-50 flex h-full w-full items-center justify-center p-4">
            <motion.div
              ...
              className="pointer-events-auto relative w-full max-w-[420px] rounded-2xl border border-zinc-200 bg-white p-8 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950"
            >
              <button
                type="button"
                onClick={onClose}
