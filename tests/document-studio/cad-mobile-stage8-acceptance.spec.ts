...
async function uploadFixture(page: Page): Promise<string> {
  ...
  return page.evaluate(async ({ content }) => {
    ...
    formData.append(
      "file",
      new File([content], "stage8-r12-mobile-precision.dxf", {
        type: "application/dxf",
      })
    );
    formData.append("pathname", `cad-stage8-r12-${crypto.randomUUID()}.dxf`);
    ...
    if (!response.ok || !payload.file?.id) {
      throw new Error(payload.error || "Stage 8 R12 DXF fixture yüklenemedi");
    }
...
test("Stage 8: beş snap modu geçerli R12 DXF üzerinde ayrı ayrı çözülür ve ayar kalıcıdır", async ({ page }) => {
...
test("Stage 8: long-press, büyüteç, offset crosshair, rubber-band ve iki nokta commit birlikte çalışır", async ({ page }) => {
  const { host, viewport, center } = await openViewer(page);
...
test("Stage 8: ikinci parmak pending long-press ölçümünü iptal eder", async ({ page, browserName }) => {
  test.skip(
    browserName === "webkit",
...
test("Stage 8: trusted iki-parmak pinch doğal hızda zoom yapar ve merkez ankrajını korur", async ({ page, browserName }) => {
  test.skip(
