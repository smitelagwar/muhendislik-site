...
assert.equal(multiTouchStarts, 0, "one touch must remain a single-pointer gesture");
assert.equal(guard.isMultiTouchActive, false);

host.emit("pointerdown", 90, "mouse");
assert.equal(multiTouchStarts, 0, "mouse pointers must not join the mobile gesture set");

host.emit("pointerdown", 2, "touch");
assert.equal(multiTouchStarts, 1, "the second touch must start multi-touch exactly once");
assert.equal(guard.isMultiTouchActive, true);

host.emit("pointerdown", 3, "touch");
assert.equal(multiTouchStarts, 1, "additional fingers must not repeatedly cancel a command");
assert.equal(guard.isMultiTouchActive, true);
...
assert.equal(multiTouchStarts, 2, "a later second touch must start a new multi-touch gesture");
assert.equal(guard.isMultiTouchActive, true);

host.emit("lostpointercapture", 4, "touch");
assert.equal(guard.isMultiTouchActive, false, "lost capture must clean up touch state");
...
for (const token of [
  ...
  assert.ok(adapter.includes(token), `adapter gesture contract token missing: ${token}`);
}
