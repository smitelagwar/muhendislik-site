...
import { DEPREM_PHASE7_SLUGS } from "../src/lib/deprem-phase7-articles";
...
for (const spec of DEPREM_ROLLOUT_ARTICLES) {
  ...
  const expectedUpdatedAt = DEPREM_PHASE7_SLUGS.has(article.slug) || DEPREM_PHASE6_SLUGS.has(article.slug) || DEPREM_PHASE5_SLUGS.has(article.slug) || DEPREM_PHASE4_SLUGS.has(article.slug)
    ? "26 Ağustos 2026"
    : "25 Ağustos 2026";
...
console.log(JSON.stringify({
  ...
  updatedAtOwnership: "FAZ 4/5/6/7 canonical technical source-of-truth owns 26 Ağustos 2026; older rollout-only records retain 25 Ağustos 2026",
