                ...
                >
                  <div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="rounded-md border border-amber-400/25 bg-amber-400/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-amber-200">
                        {standard.code}
                      </span>
                      <ArrowRight className="h-4 w-4 text-slate-500 transition-transform duration-300 group-hover:translate-x-1 group-hover:text-white" />
                    </div>
                    <h3 className="mt-5 text-xl font-black text-white">{standard.title}</h3>
                    <p className="mt-3 text-sm leading-7 text-slate-300">{standard.description}</p>
                  </div>
                  <p className="mt-5 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{standard.note}</p>
                </Link>
              </AnimatedSection>
            ))}
          </div>
        ...
        <div>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h3 className="mt-3 text-3xl font-black tracking-tight text-white">Kazıdan teslime tek bakış</h3>
            </div>
            <GitBranchPlus className="h-5 w-5 text-cyan-300" />
          </div>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {phasePreviews.map((phase, index) => (
              <AnimatedSection key={phase.id} animation="fade-up" delay={index * 70}>
                <Link
                  href={phase.href}
                  className="group flex h-full gap-4 rounded-lg border border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.66),rgba(7,11,18,0.92))] p-4"
                ...
                >
                  ...
                  <div className="min-w-0">
                    <h4 className="text-base font-black text-white transition-colors group-hover:text-cyan-200">
                      {phase.title}
                    </h4>
                    <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-300">{phase.summary}</p>
                  </div>
                </Link>
              </AnimatedSection>
            ))}
