          ...
          >
            {groups.map((group) => (
              <TabsTrigger
                key={group.id}
                value={group.id}
                className="rounded-md border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/5 px-4 py-2.5 text-sm font-semibold text-slate-600 dark:text-slate-300 data-[state=active]:border-amber-400/30 data-[state=active]:bg-amber-500/10 dark:bg-amber-400/10 data-[state=active]:text-slate-900 dark:data-[state=active]:text-white"
              >
                {group.label}
              </TabsTrigger>
            ))}
          ...
          <TabsContent key={group.id} value={group.id} className="outline-none">
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_21rem]">
              <div className="grid gap-4 md:grid-cols-2">
                {group.articles.map((article, index) => (
                  <AnimatedSection key={article.slug} animation="fade-up" delay={index * 70}>
                    <Link
                      href={`/${article.slug}`}
                      className="group flex h-full flex-col overflow-hidden rounded-[1.25rem] home-glass-panel"
                    >
                      ...
                      <div className="flex flex-1 flex-col justify-between p-5">
                        <div>
                          ...
                          <h3 className="mt-4 text-xl font-black leading-snug text-slate-900 dark:text-white transition-colors group-hover:text-amber-600 dark:text-amber-200">
                            {article.title}
                          </h3>
                          <p className="mt-3 line-clamp-3 text-sm leading-7 text-slate-600 dark:text-slate-300">{article.description}</p>
                        </div>

                        <div className="mt-6 flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
              ...
              <AnimatedSection animation="fade-up" className="h-fit">
                <aside className="rounded-[1.25rem] border border-amber-300 dark:border-white/10 bg-gradient-to-b from-amber-50 to-white dark:from-amber-900/20 dark:to-[#070b12] shadow-sm shadow-amber-200 dark:shadow-none p-6">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400 dark:text-slate-400">{group.label}</p>
                  <div className="mt-4 font-mono text-4xl font-black text-slate-900 dark:text-white">{group.totalCount}+</div>
